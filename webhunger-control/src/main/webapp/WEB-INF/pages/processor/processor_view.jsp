...
<div class="container">
    ...
    <table id="processor_table" class="table table-hover" cellspacing="0" width="100%">
        <thead>
        <tr>
            <th width="4%">#</th>
            <th>Host</th>
...
<script type="text/javascript">
    ...
    $("#processorMenu").addClass("active");

    processorTable = $("#processor_table").DataTable({
        "ajax": {
            "url": "${AppContext}processor/list",
            "type": "POST"
        },
        "columns": [
            {"data": "hostName"},
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    var buttonStr = "";
                    if (data === 0) {
                        buttonStr += "<button type='button' class='btn btn-default btn-sm' onclick='runProcessor(\""+row.ip+"\")'>Run</button> ";
                    } else {
    ...
    function updateProcessorTable() {
        processorTable.ajax.reload();
    }
    
    function runProcessor(processorIP) {
        $.ajax({
            "url": "${AppContext}processor/" + processorIP + "/run",
            "type": "POST",
            "success": function (data) {
                data = JSON.parse(data);
                if (data.res_code === 0) {
                    updateProcessorTable();
                    $("#modalContent").html("You have added a processor to run successfully~");
